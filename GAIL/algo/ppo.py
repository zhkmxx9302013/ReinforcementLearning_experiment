...
class PPOTrain:
    ...
    def train(self, obs, actions, gaes, rewards, v_preds_next):
        feed_dict = {
            self.Policy.obs_agent: [obs[0][0:20]],
            self.Policy.obs_other: [obs[0][-7:]],
            self.Old_Policy.obs_agent:  [obs[0][0:20]],
            self.Old_Policy.obs_other: [obs[0][-7:]],
    ...
    def log_parameter(self, obs, actions, gaes, rewards, v_preds_next):
        ...
        # self.gaes: gaes
        # }
        lc, ent, lvf, loss = tf.get_default_session().run([self.loss_clip, self.entropy, self.loss_vf, self.loss], feed_dict=feed_dict)
    ...
    def get_grad(self, obs, actions, gaes, rewards, v_preds_next):
        ...
        # self.gaes: gaes
        # }
        return tf.get_default_session().run(self.gradients, feed_dict=feed_dict)
